  ...
  collection.find({} ,{}).toArray(function(e, results){
    if (e)res.status(500).send()
        ...
        if(results[0].paypal.valueOf()=="none".valueOf())
        {
          ...
          res.send("paypal is linked")
          })
        }
        else
        {
          console.log("has pyapal already")
          res.send("cant change it")
        }
    }
  })
})
  ...
  for(var i = 0; i < req.body.friendarray.length; i++)
     {
      ...
      personalDictionary["items"] = req.body.items;
      personalDictionary["prices"] = req.body.prices;
      personalDictionary["ownerid"] = req.body.ownerid;
      console.log(req.body.friendarray)
